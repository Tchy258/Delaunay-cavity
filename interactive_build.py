...
# Macro options
# -------------------------------
MESH_GENERATOR_OPTIONS = ["DelaunayCavityGenerator", "PolyllaGenerator"]
...
SELECTION_CRITERION_T_OPTIONS = ["NullSelectionCriterion", "MinAngleCriterionRobust", "MinAngleCriterion", "MinAreaCriterion", "MinArea2Criterion"]
...
def build_target(generator, mesh, tri_base, merge, selection_criterion, sort_order_ascending, sort_key_val):
    # Use hyphens between options and snake_case
    parts = [generator,mesh, tri_base, merge, selection_criterion]
    if tri_base != "NullComparator" and tri_base != "RandomComparator":
        parts = [generator,mesh, (("Ascending") if sort_order_ascending else ("Descending")) + (COMPARATOR_SORT_KEYS[tri_base][1] if sort_key_val else COMPARATOR_SORT_KEYS[tri_base][0]) + tri_base , merge, selection_criterion]
    ...
    cmake_cmd = [
        ...
        f"-DGENERATOR_T={generator}",
        f"-DMESH_TYPE={mesh}",
        f"-DTRIANGLE_COMPARATOR_BASE_T={tri_base}",
        f"-DMERGING_STRATEGY_T={merge}",
        f"-DSELECTION_CRITERION_T={selection_criterion}",
    ]
    if generator == "DelaunayCavityGenerator":
        if tri_base != "NullComparator" and tri_base != "RandomComparator":
            ...
            cmake_cmd.append("-DDELAUNAY_GENERATOR=1")
        if selection_criterion != "NullSelectionCriterion":
            cmake_cmd.append("-DSELECTION_CRITERION_WITH_ARG=1")
    cmake_cmd.append("..")
    subprocess.run(cmake_cmd, cwd=BUILD_DIR, check=True)
    subprocess.run(["cmake", "--build", ".", "--target", target_name], cwd=BUILD_DIR, check=True)

def build_polylla(generator,mesh):
    cmake_cmd = [
            "cmake",
            "-DBUILD_MACRO_COMBO=ON",
            f"-DGENERATOR_T={generator}",
    ...
    subprocess.run(cmake_cmd, cwd=BUILD_DIR, check=True)
    parts_snake = [to_snake_case(p) for p in [generator, mesh]]
    target_name = "-".join(parts_snake)
    subprocess.run(["cmake", "--build", ".", "--target", target_name], cwd=BUILD_DIR, check=True)
# -------------------------------
# Interactive selection
# -------------------------------
generator = choose_option("GENERATOR_T", MESH_GENERATOR_OPTIONS)
if generator != "all":
    mesh = choose_option("MESH_TYPE", MESH_TYPE_OPTIONS)
else:
    mesh = "all"
if generator != "PolyllaGenerator":
    if not "all" in [generator,mesh]:
    ...
    if tri_base != "NullComparator" and tri_base != "RandomComparator" and tri_base != "all":
        ...
        if sort_key == COMPARATOR_SORT_KEYS[tri_base][0]:
            ...
            sort_key_val = True
    if not "all" in [generator, mesh, tri_base]:
        merge = choose_option("MERGING_STRATEGY_T", MERGING_STRATEGY_T_OPTIONS)
    else:
        merge = "all"
    if not "all" in [generator, mesh, tri_base, merge]:
        selection_criterion = choose_option("SELECTION_CRITERION_T", SELECTION_CRITERION_T_OPTIONS)
    else:
        selection_criterion = "all"
# -------------------------------
# Determine build mode
# -------------------------------
if "all" in [generator, mesh, tri_base, ascending_val, sort_key_val, merge, selection_criterion]:
    ...
    if generator != "PolyllaGenerator":
        build_target(generator, mesh, tri_base, merge, selection_criterion, ascending_val, sort_key_val)
    else:
        build_polylla(generator, mesh)
